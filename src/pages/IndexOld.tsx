...
import { ArrowLeft, Menu, Upload, FileText, Zap } from 'lucide-react';
...
const Index = () => {
  ...
  const handleGenerateRequest = async (request: AIGenerationRequest) => {
    logStage('ai_generate_request', { format: request.format, hasFiles: !!request.uploadedFiles?.length });

    // If there are uploaded files, process them through the durable state pipeline
    if (request.uploadedFiles && request.uploadedFiles.length > 0) {
      await handleFileUpload(request.uploadedFiles);
      return; // handleFileUpload will navigate to class-builder
    }

    // For AI-only generation (no files), navigate with request
    navigate('/class-builder', { state: { request } });
  };

  // Handle file uploads through durable state pipeline
  const handleFileUpload = async (files: File[]) => {
    logStage('file_upload_start', { fileCount: files.length, fileNames: files.map(f => f.name) });
    ...
    try {
      ...
      if (!extractionResult.success || !extractionResult.combinedText) {
        toast({
          ...
          description: `Could not extract text from files: ${extractionResult.errors.join(', ')}`,
          variant: "destructive",
        });
        return;
      }

      // Store extracted text in durable state
      const { setUploadedText, setNormalizedText, setBaselinePlan, reset } = usePlanStore.getState();
      ...
      // Normalize the text
      const normalizedText = normalizeUploadedFile(extractionResult.combinedText);
      setNormalizedText(normalizedText);

      // Parse into baseline plan
      const parseResult = parseUploadedPlan(normalizedText);

      if (!parseResult.success || !parseResult.plan || !parseResult.plan.blocks?.length) {
        toast({
          title: "Parsing Failed",
          description: parseResult.error || "Could not parse workout structure from uploaded files.",
          variant: "destructive",
        });
        return;
      }
      ...
      // Store baseline plan
      setBaselinePlan(parseResult.plan, parseResult.warnings);

      toast({
        title: "Files Processed Successfully",
        description: `Extracted ${parseResult.plan.blocks.length} workout blocks. Ready to generate!`,
      });

      // Navigate to class builder (Review page)
      navigate('/class-builder');

    } catch (error) {
      console.error('Error processing uploaded files:', error);
      logStage('file_upload_error', { error: error instanceof Error ? error.message : 'Unknown error' });
